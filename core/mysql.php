...
function atualiza(string $entidade, array $dados, array $criterio = []) : bool {
    ...
    foreach ($criterio as $expressao) {
        ...
        $coringa_criterio[] = implode(' ', $expressao);

        $nome_campo = (count($expressao) < 3) 
            ? $expressao[0] 
            : $expressao[0] . $expressao[1];

        if (isset($$nome_campo)) {$$nome_campo . '_' . rand();
        }
...
function buscar(string $entidade, array $campos = ['*'], array $criterio = [], string $ordem = null) : array {
    ...
    if ($resultado = mysqli_stmt_get_result($stmt)) {
        $retorno = mysqli_fetch_all($resultado, MYSQLI_ASSOC);
    }
